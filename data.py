...
def load(file_names, year, month, output_folder):
    ...
    contracheque = _read(_convert_file([c for c in file_names if "contracheque" in c][0]))
    indenizatorias = _read(_convert_file([i for i in file_names if "indenizatorias" in i][0]))
...
class Data:
    ...
    def validate(self):
        ...
        if not (
                ...
                or os.path.isfile(
            f"{self.output_folder}/membros-ativos-indenizatorias-{self.month}-{self.year}.xlsx"
        )
        ):
